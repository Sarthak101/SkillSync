...
// Fetch technicians associated with the admin
if(isset($_SESSION['rEmail'])){
  $adminEmail = $_SESSION['rEmail'];
  $sqlTechnicians = "SELECT empid, empName FROM technician_tb WHERE ngo_id = (SELECT a_login_id FROM adminlogin_tb WHERE a_email = '$adminEmail')";
  $resultTechnicians = $conn->query($sqlTechnicians);

  // Check if there are no technicians
  $noTechnicians = false;
  ...
  <form action="" method="POST">
    ...
    <div class="form-row">
      <div class="form-group col-md-6">
        ...
        <?php if($noTechnicians): ?>
          <div class="alert alert-info" role="alert">
            No employees available. Please add them from Employess section.
          </div>
        <?php else: ?>
          <select class="form-control" id="assigntech" name="assigntech">
            <option value="">Select Technician</option>
            <?php
            if(isset($resultTechnicians)) {
              while($technician = $resultTechnicians->fetch_assoc()) {
                echo "<option value='{$technician['empid']}'>{$technician['empName']}</option>";
              }
            }
            ?>
          </select>
